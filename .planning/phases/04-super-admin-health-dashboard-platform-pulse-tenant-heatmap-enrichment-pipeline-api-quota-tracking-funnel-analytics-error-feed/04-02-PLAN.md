...
must_haves:
  truths:
    ...
    - "Super admin can fetch recent enrichment failures with prospect name, tenant, error details, and timestamp"
...
2. Fetch user counts per tenant: `users` grouped by `tenant_id` — `SELECT tenant_id, COUNT(*)`.
3. Fetch 7d usage metrics: `usage_metrics_daily WHERE date >= 7 days ago` — group by `tenant_id`, SUM `searches_executed`, `profiles_enriched`, `csv_exports`.
4. Fetch last active per tenant: `activity_log` — `SELECT tenant_id, MAX(created_at)`. Include `created_at >= 90 days ago` to limit partition scan.

Aggregation: In-memory join of all 4 queries by tenant_id. Return array sorted by total searches descending (most active first).
...
Query: `prospects` table — `SELECT id, full_name, enrichment_status, enrichment_source_status, updated_at, tenant_id` WHERE `enrichment_status = 'failed'` AND `updated_at >= 7 days ago`. Order by `updated_at DESC`. Use `.range()` for pagination.
...
Note: Per CONTEXT.md locked decision — NO re-trigger button on error feed. This is view-only. The API does not need a POST/PUT endpoint for re-triggering.
  ...
  <done>Three API routes created: /api/admin/enrichment/health returns per-source daily breakdown, /api/admin/funnel returns 4-stage funnel counts, /api/admin/errors returns paginated failed enrichments with per-source error details. All handle both string and object enrichment_source_status entries.</done>
</task>
