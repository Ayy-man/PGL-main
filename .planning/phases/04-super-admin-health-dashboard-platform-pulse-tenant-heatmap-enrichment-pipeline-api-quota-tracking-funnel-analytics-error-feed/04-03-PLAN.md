...
must_haves:
  truths:
    ...
    - "Tenant rows expand inline to show per-user breakdown on click, with a link to the tenant admin page"
    ...
    - "Error feed is a paginated expandable table showing failed enrichments with User column, per-source error details, and prospect name linking to prospect profile"
...
**Secondary navigation** (CONTEXT.md decision): In the expanded row, include a link to the tenant's admin page. Render the tenant name as a clickable `Link` to `/admin/tenants/${tenant.id}` (or the appropriate tenant admin route). Use Next.js `Link` component with `text-primary hover:underline` styling so the super admin can navigate to the tenant's detail page for deeper investigation.
...
Mark with `"use client"` (uses hooks for expand state).
  ...
  <done>useCountUp hook, ComingSoonCard wrapper, PlatformPulse stat cards (4 cards with count-up, Coming Soon for quota), and TenantHeatmap (expandable table with relative-to-peers color coding, tenant admin page link in expanded row) all created and type-check.</done>
</task>
...
Render a table with columns: Timestamp, Tenant, User, Prospect, Status, Details (expandable).

The "User" column displays `userName` (from DESIGN.md Section 4: "User — Join to users.full_name") showing which user triggered the enrichment.
...
- **Secondary navigation** (CONTEXT.md decision): The Prospect name in each row should be a clickable link to the prospect profile page: `/${tenantId}/prospects/${id}`. Use Next.js `Link` component. This allows the super admin to drill from a failed enrichment into the prospect's profile for investigation.
...
Mark with `"use client"`.
  ...
  <done>EnrichmentHealthChart (stacked bar with gold-adjacent colors), FunnelChart (4-stage funnel with gold-to-amber gradient), and ErrorFeed (paginated expandable table with User column, prospect profile links, view-only) all created and type-check. Charts use Recharts from installed package.</done>
</task>
...
- TenantHeatmap uses relative-to-peers ranking (no hardcoded thresholds), new tenants show "New" not red, expanded rows link to tenant admin page
...
- ErrorFeed is paginated, expandable, view-only (no re-trigger button), includes User column, prospect names link to prospect profile
