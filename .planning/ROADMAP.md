...
**Goal:** Close the security hole surfaced by Phase 39 VERIFICATION: four write API surfaces (`src/app/[orgId]/lists/actions.ts`, `src/app/[orgId]/personas/actions.ts`, `src/app/api/apollo/bulk-enrich/route.ts`, `src/app/api/prospects/[id]/notes/route.ts`) currently enforce the Assistant read-only role only on the client via `canEdit` prop. An authenticated Assistant user can bypass the UI and POST directly to these endpoints — deleting lists, mutating personas, burning Apollo credits on bulk-enrich, or writing notes. Before Maggie's team logs in with mixed roles, every write on those routes must server-side reject Assistants with a 403 and a consistent error shape. Deliverable: (1) extract a `requireWriteRole()` helper that checks `session.user.app_metadata.role` against `['super_admin', 'tenant_admin', 'agent']` (mirror the existing `requireSuperAdmin()` pattern — see Phase 04 decision: inline auth check in Route Handlers, not redirect-based); (2) apply it at the top of every mutating handler in the 4 files — including Server Actions, not just Route Handlers; (3) return `{ error: 'FORBIDDEN', message: 'Your role does not permit this action' }` with status 403; (4) Vitest integration tests per route: assistant POST → 403, agent/admin POST → 200, unauthenticated → 401 (preserve existing auth behavior); (5) grep-zero check that no mutating handler in those 4 files is missing the role guard. No UI changes — Phase 39-03 already threads `canEdit` client-side; this phase makes the server honor the same contract. Scope is strictly the 4 files identified in the audit — do not expand to re-audit the entire API surface (that's a separate phase).

**Requirements:** PHASE-42-HELPER (new `requireWriteRole()` in `src/lib/auth/`), PHASE-42-LISTS (guard all mutations in `lists/actions.ts`), PHASE-42-PERSONAS (guard all mutations in `personas/actions.ts`), PHASE-42-BULK-ENRICH (guard POST in `apollo/bulk-enrich/route.ts` — highest cost risk), PHASE-42-NOTES (guard POST/PATCH/DELETE in `prospects/[id]/notes/route.ts`), PHASE-42-ERROR-SHAPE (consistent 403 response across all 4), PHASE-42-TESTS (Vitest integration tests — assistant 403 / agent 200 / unauth 401), PHASE-42-GREP-VERIFY (ripgrep zero-match for unguarded mutations in scoped files), PHASE-42-UAT (manual DevTools spot-check as assistant role)
**Depends on:** Phase 39
**Status:** Not started — pre-Maggie-demo blocker
...
Plans:
- [x] 44-01-PLAN.md — Migration (visibility_mode enum, ALTER TABLE, RLS policies) + shared Visibility type + [BLOCKING] schema push
- [x] 44-02-PLAN.md — Type extensions (List, Persona) + query-layer additions (updateListVisibility, getAllListsWithCreators, persona mirrors, seedStarterPersonas visibility override)
- [x] 44-03-PLAN.md — Server actions (visibility on create, updateListVisibilityAction, updatePersonaVisibilityAction) + Vitest coverage
- [x] 44-04-PLAN.md — Creation dialog visibility segmented control (create-list-dialog, persona-form-dialog) + makeTempList thread-through + optimistic reducer test
- [x] 44-05-PLAN.md — Visibility badges + creator-or-admin-gated toggle dropdown (list-grid, list detail, persona-card)
