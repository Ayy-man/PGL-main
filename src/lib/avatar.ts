/**
 * Avatar utility — lightweight MD5 hash + Gravatar URL cascade.
 *
 * Cascade: photoUrl -> Gravatar -> null (caller renders initials fallback)
 */

// Lightweight MD5 implementation (browser + Node compatible, no dependencies)
function md5(input: string): string {
  ...
  function wordsToHex(words: number[]): string {
    ...
    for (let i = 0; i < 4; i++) {
      const w = words[i];
      for (let j = 0; j < 4; j++) {
        hex += ((w >> (j * 8 + 4)) & 0xf).toString(16);
        hex += ((w >> (j * 8)) & 0xf).toString(16);
      }
    }
    return hex;
  }
  ...
  const result = computeMd5(words, bytes.length * 8);
  return wordsToHex(result);
}

/**
 * Returns the best available avatar URL for a prospect.
 *
 * Cascade:
 * 1. photoUrl if non-empty — return directly
 * 2. email if non-empty — return Gravatar URL (d=404 so onError can catch missing avatars)
...
export function getAvatarUrl(params: {
  ...
  const { photoUrl, email, size = 80 } = params;
